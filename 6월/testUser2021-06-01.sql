...
AS
(
SELECT B.bk_isbn, B.bk_title AS 도서명, 
C.cp_name AS 회사명, C.cp_ceo AS ,
A.au_name AS 저자명, A.au_tel AS 전화번호,
B.bk_date AS 출판일 
FROM tbl_books B
    LEFT JOIN tbl_company C
        ON B.bk_comp = C.cp_code
...
MY SQL : LEFT(문자열 칼럼, 앞에서 몇개)
*/
  
-- tbl_books와 tbl_company, tbl_books와 tbl_author FK설정
-- bk_comp와 cp_code     bk_author와 au_code
ALTER table tbl_books       -- 누구한테
ADD CONSTRAINT fk_company   -- 이름
...
-- 수정이상, 삭제이상을 방지하는 방법 
-- 중복된데이터는 절대 추가될 수 없다 : 삽입이상을 방지하는 방법


-- FK 
-- 두개이상의 table을 연결하여 view(조회)를 할때
-- 어떤 데이터가 NULL 값으로 보이는 것을 방지하기 위한 조치이다.

-- Child(tbl_books):bk_comp        Parent(tbl_company):cp_code
--  있을 수 있고, 추가가능    <<          있는 코드
-- 있어서는 안되고 추가불가능 <<          없는 코드
--   있는 코드                >>          코드 삭제 불가능
--   있는 코드                >>          반드시 있어야 한다.

-- 리처드쇼튼의 연락처가 010-6676-6428에서 
...
-- 정보를 수정, 삭제하는 절차
-- 내가 수정, 삭제 하고자하는 데이터가 어떤 상태인지 조회

SELECT *
FROM tbl_author
WHERE au_name LIKE '%리%';
