...
-- 생성한 TAble에 데이터 추가
-- DML(데이터 조작어) 명령어를 사용하여 데이터를 추가(create(DDL))
...
-- 같은 학번의 데이터가 이미 추가되어 있어도 또 다시 추가가 되어버린다.
...
-- 1. 학생의 이름은 데이터가 반드시 있어야 만 한다.(NOT NULL)
--      st_name(학생이름) 컬럼은 NOT NULL 이어야 한다.
-- 2. 학번은 절대 중복되면 안된다.(UNIQUE) (NOT NULL)
--      tbl_student 테이블에 모든 데이터의 학번은 유일해야 한다.
-- 필수적으로 있어야할 데이터는 NOT NULL 그리고 겹치면 안되니깐 UNIQUE
