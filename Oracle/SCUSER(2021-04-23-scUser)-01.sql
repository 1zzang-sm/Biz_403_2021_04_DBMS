...
CREATE TABLESPACE tbl_score
DATAFILE 'C:/oraclexe/data/score.dbf'
SIZE 1M AUTOEXTEND ON NEXT 1K;

DROP TABLESPACE tbl_score
...
-- CREATE로 작성한 칼럼의 순서대로 모든 데이터를 
-- 포함하여 INSERT수행하기
-- 항상 CREATE로 작성한 칼럼의 순서를 기억해야 하고
--      순서가 바뀌면 전혀 엉뚱한 데이터가 INSERT가 될 수 있다.
-- 모든 칼럼에 데이터를 포함해야 한다.
...
-- 수행할 수 있다.
INSERT INTO tbl_score(st_kor, st_eng, st_math, st_num)
...
--      NOT NULL UNIQUE : PK(Primary Key)로 선언하는 것도 좋은 방법
...
CREATE TABLE tbl_score(
    ...
    PRIMARY KEY (sc_num) -- pk를 마지막에 선언 할 수 있다.
);
