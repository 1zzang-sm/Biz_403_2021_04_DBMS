...
--      books의 bk_code 와 company의 cp_code
--      books의 bk_acode와 author의 au_code
...
FROM tbl_books, tbl_author, tbl_company
    WHERE bk_code = cp_code AND bk_acode = au_code;
    
-- 다수의 table이 연관(Relation) 관계에 있을때 
-- JOIN 결과가 잘못되면 DB신뢰성에 매우 큰 문제가 발생한다.
