...
-- 전체 상품리스트와 매입,매출 단가를 조회하기
-- 같은 상품이라도 거래 시기에 따라 매입과 매출금액이
-- 달라질 수 있기 때문에 데이터중에서 제일 높은 단가를 가져오기
SELECT io_pname,
...
/*
    매입 매출 데이터로부터 상품정보 테이블 데이터를 생성하기
    ...
    4. 매입단가가 0인 데이터는 매출데이터에서 임의로 생성하기
        ...
        E2 항목의 값을 0.8을 곱하여 E2의 80% 가격이 되고 
...
/*
    ...
    매입매출데이터와, 상품정보, 거래처정보 
    JOIN하기 위해서는 매입매출 데이터에 상품코드, 거래처코드가 있어야 한다
